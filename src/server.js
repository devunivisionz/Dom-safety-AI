...
app.use('/artifacts', express.static(tmpdir(), {
  fallthrough: false
}));
...
const KNOWN_PROJECT_OPTIONS = ['Bauxite (BW150)',
'Bauxite II (BWI110)',
'Bauxite III (BWI100)',
'Cinco',
'Temple',
'Temple Stampede'];
const KNOWN_HAZARD_OPTIONS = ['Aerial Lifts/MEWP (Plataformas elevadoras (MEWP))',
...
function splitDateTime(d, t) {
  ...
  const rd = clean(d),
  rt = clean(t);
  const iso = rd.match(/\d {
    4
  }-\d {
    2
  }-\d {
    2
  }/)?.[0] || fb.toISOString().slice(0, 10);
...
function normalizePayload(rawBody) {
  ...
  return {
    ...
    photo_filename: clean(body.photo_filename) || (clean(body.record_id) ? safety-observation-$ {
      clean(body.record_id)
    }.jpg : 'safety-observation.jpg'),
...
function byLabel(page, label) {
  return page.getByLabel(label, {
    exact: true
  }).or(page.getByLabel(labelRegex(label))).first();

}function comboByLabel(page, label) {
  return byLabel(page, label).or(page.getByRole('combobox', {
    name: label, exact: true
  })).or(page.getByRole('combobox', {
    name: labelRegex(label)
  })).first();
...
async function fillText(page, label, value) {
  ...
  const labelLocator = page.getByText(label, {
    exact: true
  }).or(page.getByText(labelRegex(label))).first();
  ...
  try {
    ...
    const inputs = Array.from(document.querySelectorAll('input([type="hidden"]), textarea'));
...
async function clickVisibleText(page, targetValue, options = {
  ...
  return page.evaluate(({
    ...
    const vn = nodes.map((n) => {
      ...
      return {
        node: n, text: t, lowerText: t.toLowerCase(), style: s, box: b
      };
    ...
    let match = null;
    if (preferExact) match = vn.find(({
      text
    }) => text === target);
    if (!match && allowPartial) match = vn.find(({
      lowerText
    }) => lowerText.includes(lt) || lt.includes(lowerText));
...
async function chooseFirstMatchingFallback(page, fallbackValues = []) {
  for (const fv of fallbackValues) {
    const c = await clickVisibleText(page, fv, {
      maxTextLength: 160, allowPartial: true, preferExact: true
    }).catch(() => false);
    if (c) {
...
async function clickFirstSmallOption(page) {
  return page.evaluate(() => {
    ...
    const c = nodes.map((n) => ({
      node: n, style: window.getComputedStyle(n), box: n.getBoundingClientRect(), text: normalize(n.textContent)
    })).filter(({
      style, box, text
    }) => style.visibility !== 'hidden' && style.display !== 'none' && box.width > 0 && box.height > 0 && text && text.length <= 120 && !/submit|clear form|report malicious|do not submit/i.test(text));
    const cand = c[0];
    if (!cand) return '';
    cand.node.scrollIntoView({
      block: 'center'
    });
    cand.node.click();
    return cand.text;
...
async function chooseLinkedRecord(page, value, addNames, label, fallbackValues = []) {
  ...
  console.log([$ {
    label
  }] selecting linked record:, value);
  await dismissOpenPopover(page);
  const fieldLabel = page.getByText(label, {
    exact: true
  }).or(page.getByText(labelRegex(label))).first();
  ...
  for (const an of addNames) {
    const r = new RegExp('\+?\sAdd\s+.' + escapeRegExp(an), 'i');
    ...
    const dc = await clickVisibleText(page, value, {
      maxTextLength: 160, allowPartial: true, preferExact: true
    }).catch(() => false);
    if (dc) {
      await page.waitForTimeout(700);
  ...
  await addButton.click({
    timeout: 5000, noWaitAfter: true, force: true
  });
  await page.waitForTimeout(1000);

  
  const vt = [value,
  ...fallbackValues.filter((x) => x && x !== value)];
  for (const av of vt) {
    await setSearchInputValue(page, av);
    await page.waitForTimeout(1000);
    const c = await clickVisibleText(page, av, {
      maxTextLength: 160, allowPartial: true, preferExact: true
    }).catch(() => false);
    if (c) {
      await page.waitForTimeout(700);
...
async function chooseLinkedProject(page, value) {
  return chooseLinkedRecord(page, value || FIELD_DEFAULTS.project_site, ['project'], 'Project Site', [value,
  FIELD_DEFAULTS.project_site,
  ...KNOWN_PROJECT_OPTIONS].filter(Boolean));
...
async function chooseComboByPartialMatch(page, label, value, fallbackValues = []) {
  ...
  console.log([$ {
    label
  }] choosing dropdown value:, value);
  const combo = comboByLabel(page, label);
  await combo.scrollIntoViewIfNeeded({
    timeout: ACTION_TIMEOUT_MS
  });
  await combo.click({
    timeout: ACTION_TIMEOUT_MS, noWaitAfter: true, force: true
  });
  await page.waitForTimeout(700);
  ...
  for (const av of vt) {
    await setSearchInputValue(page, av);
    await page.keyboard.type(String(av), {
      delay: 10
    }).catch(() => undefined);
    await page.waitForTimeout(800);
    const c = await clickVisibleText(page, av, {
      maxTextLength: 180, allowPartial: true, preferExact: true
    }).catch(() => false);
    if (c) {
      await page.waitForTimeout(700);
      await page.keyboard.press('Escape').catch(() => undefined);
  ...
  console.warn(No dropdown option found for "${label}" value "${value}". Continuing. Visible: $ {
...
async function withFallback(page, {
  fieldName, value, defaultValue, primaryFn, fallbackFn, fallbacksUsed, warn = console.warn
}) {
  try {
    ...
    fallbacksUsed.push({
      field: fieldName, tried: value, usedDefault: defaultValue || null, error: e.message
    });
    await dismissOpenPopover(page);
    if (defaultValue) {
      try {
...
async function pickDate(page, iso) {
  ...
  if (!hhmm) return '';
  const [h,
  m] = hhmm.split(':').map(Number);
  return setInputValueByPlaceholder(page, 'hh', airtableTimeLabel(h, m));
...
async function safeScreenshot(page, p) {
  ...
  try {
    await withTimeout(page.screenshot({
      path: p, fullPage: false, timeout: SCREENSHOT_TIMEOUT_MS
    }), SCREENSHOT_TIMEOUT_MS + 1000, 'Timed out capturing screenshot');
    return p;
...
async function fillForm(payload, req, tracker = {
  stage: 'initializing'
}) {
  ...
  const withFB = (opts) => withFallback(page, {
    ...opts, fallbacksUsed
  });
  ...
  try {
    browser = await stage('launch browser', async () => playwrightChromium.launch({
      headless: true, executablePath: process.env.CHROMIUM_EXECUTABLE_PATH || (await serverlessChromium.executablePath()), args: [...serverlessChromium.args,
      '--no-sandbox',
    ...
    await stage('wait Airtable network idle', () => page.waitForLoadState('networkidle', {
      timeout: 15000
    }).catch(() => undefined));

    await stage('wait Airtable form ready', () => page.getByText(/Date\s+of\s+event/i).first().waitFor({
    ...
    if (payload.type_of_observation !== 'Positive/Safe Observation') {
       ...
       () => withFB({
         ...
         primaryFn: () => chooseComboByPartialMatch(page, 'Type of Hazard', payload.type_of_hazard, [FIELD_DEFAULTS.type_of_hazard,
...
function timeoutResult(payload, tracker) {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        success: false, submitted: false, test_mode: payload.test_mode, selected_values: payload.selected_values, fallbacks_used: [], failed_stage: tracker.stage || 'request timeout', error: 'Form automation exceeded ' + REQUEST_TIMEOUT_MS + 'ms before returning. Last stage: ' + (tracker.stage || 'unknown'), artifacts: {
...
async function submitObservationForm(req, res) {
  ...
  const result = await Promise.race([fillForm(payload,
  req,
  tracker),
  timeoutResult(payload,
  tracker)]);
...
app.get('/', (req, res) => res.json({
  ok: true, service: 'AI Safety Manager Form Service', submit_mode: SUBMIT_MODE, version: 'v22-submit-race-detection', endpoints: ['GET /health',
  'POST /submit-observation-form',
  'POST /'],
}));
app.get('/health', (req, res) => res.json({
  ok: true, submit_mode: SUBMIT_MODE, version: 'v22-submit-race-detection'
}));
